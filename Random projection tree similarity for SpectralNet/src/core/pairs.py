...
def create_pairs_from_unlabeled_data(x1, x2=None, y=None, p=None, k=2, tot_pairs=None, precomputed_knn_path='', use_approx=False, pre_shuffled=False,verbose=None):
    '''
    ...
    if pairs_Method == 'mu0':
        ...
        ASum = np.cumsum(Idx_dist,axis=1)
        # get cumulative count at each column
        ACount = np.asarray(range(Idx_dist.shape[1]))
        ACountRepeat = np.repeat(ACount[np.newaxis,:],Idx_dist.shape[0],axis=0) + 1
        # get cumulative mean at each column
        AMean = ASum / ACountRepeat
        # get mean + std at 7th nieghbour and replicate it to match size
        AMean7 = np.mean(Idx_dist[:,0:8], axis=1) + np.std(Idx_dist[:,0:8], axis=1)
        AMean7Repeat = np.repeat(AMean7[:,np.newaxis],Idx_dist.shape[1],axis=1)
        # subtract (mean + std at 7th nieghbour) from (cumulative mean)
        AAccept = AMean7Repeat - AMean;
        # for each row find the first element that is greater than mean+std
        AAcceptIndex = np.argmax(AAccept<0, axis=1)
        # each element would have a minimum of two neighbors
        AAcceptIndex = np.where(AAcceptIndex < 2, 2, AAcceptIndex)
        for i in range(n):
            # form the pairs
    ...
    elif pairs_Method == 'rpTree':
        tree = core.RPTree.BinaryTree(x1_flat)
        tree_root = tree.construct_tree(tree)
        
        leaves_array = tree_root.get_leaf_nodes()
        
        new_pos = []
        for r in range(len(leaves_array)):
            leaf_current = leaves_array[r]
            samples_in_current = leaf_current.shape[0]
            for i in range(samples_in_current):
                choices = range(i+1,samples_in_current)
                new_pos += [[leaf_current[i], leaf_current[c]] for c in choices]
        
        new_neg = []
        for r in range(len(new_pos)):
            i , j = random.sample(range(len(leaves_array)), 2)
            xi = random.randint(0,leaves_array[i].shape[0]-1)
            xj = random.randint(0,leaves_array[j].shape[0]-1)
            new_neg += [[leaves_array[i][xi], leaves_array[j][xj]]]
