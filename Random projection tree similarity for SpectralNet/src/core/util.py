...
def get_accuracy(cluster_assignments, y_true, n_clusters):
    ...
    with open(os.path.join(ProjectDir,'Results.txt'),'a') as my_file:
        #, NMI, ARI
        my_file.write(","+str(np.round(nmi_score, 4))+","+str(np.round(ari_score, 4)))
        # my_file.write("\n")
...
def print_accuracy(cluster_assignments, y_true, n_clusters, extra_identifier=''):
    ...
    with open(os.path.join(ProjectDir,'Results.txt'),'a') as my_file:
        #, accuracy
        my_file.write(","+str(np.round(accuracy, 4)))
        # my_file.write('spectralNet{} accuracy: '.format(extra_identifier) + str(np.round(accuracy, 4)))
        # my_file.write("\n")
        # my_file.write('==============================================================')
...
def get_y_preds(cluster_assignments, y_true, n_clusters):
    ...
    confusion_matrix = sklearn.metrics.confusion_matrix(y_true, y_pred, labels=None)
