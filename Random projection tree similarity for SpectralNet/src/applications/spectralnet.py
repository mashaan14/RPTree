...
from core.util import get_scale, print_accuracy, get_cluster_sols, LearningHandler, make_layer_list, train_gen, get_y_preds, get_project_root
...
def run_net(data, params):
    ...
    with open(os.path.join(ProjectDir,'Results.txt'),'a') as my_file:
        # Training pairs, Validation pairs, Total pairs
        # my_file.write("\n")
        # my_file.write("Training pairs, Validation pairs, Total pairs, NMI, ARI, accuracy")
        my_file.write("\n")
        my_file.write(str(pairs_train.shape[0])+","+str(pairs_val.shape[0])+","+str(pairs_train.shape[0]+pairs_val.shape[0]))
